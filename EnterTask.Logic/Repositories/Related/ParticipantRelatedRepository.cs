...
namespace EnterTask.Logic.Repositories.Related
{
    ...
    internal class ParticipantRelatedRepository : IRelatedRepository<Participant>
    {
        ...
        public ParticipantRelatedRepository(IServiceProvider provider)
        {
            _provider = provider;
        }

        public async Task<RepositoryResult> AddRelatedAsync<TRelated>(Participant data, TRelated related)
            where TRelated : class, IDataEntity, new()
        {
            try
            {
                var resolver = _provider.GetService<IRelatedEntityResolver<Participant, TRelated>>();
        ...
        public async Task<RepositoryResult<IEnumerable<TRelated>>> GetRelatedAsync<TRelated>(Participant data)
            where TRelated : class, IDataEntity, new()
        {
            try
            {
        ...
        public async Task<RepositoryResult> RemoveRelatedAsync<TRelated>(Participant data, TRelated related)
            where TRelated : class, IDataEntity, new()
        {
            try
            {
